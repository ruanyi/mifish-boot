...
class OkHttpConsumerTask implements Runnable {
    ...
    private KaproxyRequestMessageContainer kaproxyRequestMessageContainer =
        KaproxyRequestMessageContainer.getInstance();

    /** callback */
    private ProcessorConsumer.ConsumerCallback<KaproxyRequestMessage, String> callback;
    ...
    public OkHttpConsumerTask(OkHttpClient okHttpClient,
        ProcessorConsumer.ConsumerCallback<KaproxyRequestMessage, String> callback) {
    ...
    public void run() {
        ...
        try {
            while (!this.stopRead) {
                ...
                KaproxyRequestMessage kaproxyRequestMessage = this.kaproxyRequestMessageContainer.randomSelectOne();
                ...
                if (httpResp.getLeft() == 200) {
                    // 具体的业务处理
                    this.callback.callback(kaproxyRequestMessage, httpResp.getRight());
                }
                // 这边需要控制消费速度
