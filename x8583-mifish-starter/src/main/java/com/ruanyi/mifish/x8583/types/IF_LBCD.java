 ...
 * @time:2013-06-21
 */
public class IF_LBCD extends IF_ISOType implements Paddable {

    private String paddDirect = "RIGHT";

    protected byte paddByte = " ".getBytes()[0];

    public IF_LBCD(Integer length) {
        super(length);
    }
    ...
    public ISOComponent unpack(Integer index, byte[] values) throws ISOX8583Exception {
        byte lbyte = values[0];
        int msgLen = Integer.parseInt(new String(BCDUtil.unZipBcd(new byte[] {lbyte})));
        String msgStr = new String(BCDUtil.unZipBcd(BytesUtil.remove(values, 1)));
        byte[] msgBytes = null;
        try {
