package com.ruanyi.mifish.mqproxy.message.nodes;
...
import static com.ruanyi.mifish.mqproxy.model.QueueMessage.*;
...
public class RequestContextNode implements MessageNode {
    ...
    /**
     * @see MessageNode#doNode(MqproxyConsumer, QueueMessage, MessageChain)
     */
    @Override
    public MessageStatus doNode(MqproxyConsumer mqproxyConsumer, QueueMessage message, MessageChain chain) {
        try {
            // init trace_id
            String resourceUrl = AnnotationConsumerMeta.buildKey(message.getGroup(), message.getTopic());
            String traceId = UUIDUtil.obtainUUID();
            RequestContext.init(traceId, resourceUrl);
            // MDC放置：trace_id
            MDC.put("trace_id", traceId);
            // 开启方法耗时打印
            MessageStatus msgStatus = chain.doChain(mqproxyConsumer, message);
